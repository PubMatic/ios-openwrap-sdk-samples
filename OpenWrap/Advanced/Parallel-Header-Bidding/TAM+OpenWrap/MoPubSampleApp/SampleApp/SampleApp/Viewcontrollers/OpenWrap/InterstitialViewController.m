...
#define OW_INTERSTITIAL_AD_UNIT     @"61ea16f261fd4d55bb08eb8e9ddb52e3"
...
@property (nonatomic) MoPubInterstitialEventHandler *eventHandler;
...
- (void)initializeInterstitial {
    ...
    self.eventHandler = [[MoPubInterstitialEventHandler alloc] initWithAdUnitId:MOPUB_INTERSTITIAL_AD_UNIT];
    
    // Set config block on event handler instance
    __weak typeof(self) weakSelf = self;
    [self.eventHandler setConfigBlock:^(MPInterstitialAdController *interstitial, POBBid *bid) {
        interstitial.keywords = [[weakSelf.partnerTargeting allValues] componentsJoinedByString:@","];
    ...
    self.interstitial = [[POBInterstitial alloc]
                         ...
                         adUnitId:OW_INTERSTITIAL_AD_UNIT
...
- (void)didReceiveResponse:(NSDictionary *)response {
    /*!
     ...
     To send the bids targeting to MoPub, add targeting from received response in partnerTargeting dictionary. This will be sent to MoPub request using config block.
