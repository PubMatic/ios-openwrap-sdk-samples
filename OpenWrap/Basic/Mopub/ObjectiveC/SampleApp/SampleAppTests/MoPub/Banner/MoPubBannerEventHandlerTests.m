...
- (void)setUp {
    ...
    _eventHandler = [[MoPubBannerEventHandler alloc] initWithAdUnitId:MOPUB_AD_UNIT adSize:kMPPresetMaxAdSize50Height];
...
- (void)testAdServerDidWin {
    ...
    [bannerAdView.delegate adViewDidLoadAd:bannerAdView adSize:kMPPresetMaxAdSize50Height];
...
- (void)testFailedWithError {
    ...
    MPAdView *bannerAdView = self.eventHandler.bannerView;
    [bannerAdView.delegate adView:bannerAdView didFailToLoadAdWithError:nil];
    [self waitForExpectationsWithTimeout:5 handler:nil];
}
