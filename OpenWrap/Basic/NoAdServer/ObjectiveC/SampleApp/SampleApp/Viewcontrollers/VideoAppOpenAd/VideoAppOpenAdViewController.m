...
#define OW_ADUNIT_ID    @"OpenWrapAppOpenVideoAdUnit"
#define PUB_ID          @"156276"
#define PROFILE_ID      @1757

#import "VideoAppOpenAdViewController.h"
@import OpenWrapSDK;

@interface VideoAppOpenAdViewController ()<POBAppOpenAdDelegate, POBAppOpenAdVideoDelegate>
@property (nonatomic) POBAppOpenAd *appOpenAd;
@property (nonatomic) IBOutlet UIButton *showAdButton;
@end

@implementation VideoAppOpenAdViewController
...
- (void)viewDidLoad {
    ...
    // Set video delegate
    self.appOpenAd.videoDelegate = self;
}
...
- (void)appOpenAdDidExpireAd:(POBAppOpenAd *)appOpenAd{
    [self log:@"App Open Ad : Expired"];
}

#pragma mark - App Open Video Ad delegate methods

// Notifies the delegate of an video ad completion.
- (void)appOpenAdDidFinishVideoPlayback:(POBAppOpenAd *)appOpenAd{
    [self log:@"App Open Ad : Video Finished"];
}
